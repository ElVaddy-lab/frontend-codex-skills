...
description: Use when an existing website, Web/UI mockup, screenshot, Figma design, live URL, or frontend codebase must be decomposed into components, design tokens, typography, color, layout, responsive behavior, interaction patterns, assets, and implementation clues. Prefer for design-system extraction, component inventories, redesign preparation, migration analysis, and rebuild handoff. Do not use for creating a new design, implementation planning, writing code, or acceptance testing against requirements.
---
...
Load `references/input-mode-guidance.md` and state what each source can and cannot prove.
...
7. **Analyze responsiveness**: distinguish observed breakpoints from inferred adaptations. Record reflow, collapse, hide/show, reordering, density, and touch changes.
...
11. **Prepare the handoff**: state what is reusable, what must be recreated, what needs clarification, and which downstream skill should run next.
12. **Write the report**: use `references/analysis-report-template.md` and run the relevant visual or code checklist before finalizing.
